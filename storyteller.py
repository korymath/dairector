...
if __name__ == "__main__":
    import argparse    
    parser = argparse.ArgumentParser(description='Tell a story given as tree, depending on actor input')
    parser.add_argument('storytree', help='A JSON file describing a story tree')
    parser.add_argument('hintfile', help='A JSON file containing possible hints for actors')
    parser.add_argument('hintmodel', help='A trained paragraph vector model to find matches for hints')
    parser.add_argument('plotmodel', help='A trained paragraph vector model to find matches for story beats')
    parser.add_argument('-s', '--say', help='Use the say utility to produce speech output instead of printing to the console', action="store_true", default=False, dest="say")
    parser.add_argument('-n', '--names', help='A JSON file containing actor-name mappings. If not provided, a set of default names will be used', action="store", default=None, dest="names")
    parser.add_argument('-i', '--list', '--list-names', help='List the default name mapping', action="store_true", default=False, dest="listnames")
    parser.add_argument('-l', '--log', help='Store log of story in JSON file', action="store", default=None, dest="log")
    parser.add_argument('-a', '--auto', help='Automaticallz proceed to next beat', action="store_true", default=False, dest="auto")
    parser.add_argument('-p', '--speech', help='Use speech input', action="store_true", default=False, dest="speech")
    parser.add_argument('-k', '--kw', help="Use keyword 'director' to activate speech input", action="store_true", default=False, dest="kw")
    parser.add_argument('-c', '--hint-choices', help="The number of best fitting hints that should be considered and randomly chosen from (default: 5)", action="store", default=5, dest="hc")
    args = parser.parse_args()
    HINTCHOICES = int(args.hc)
    main(args.storytree, args.hintfile, args.hintmodel, args.plotmodel, args.say, args.names, args.listnames, args.log, args.auto, args.speech, args.kw)